...
#disable keras warnning about deprected issues
...
def main():
    ...
    with tf.Session(config = config) as sess:
        ...
        for epoch in range(100):
            ...
            for d, l in minibatches(train_data, train_labels, batchSize):

                _, dialogue_lengthss = pad_sequences(d, 0)
                word_idss, utterance_lengthss = pad_sequences(d, 0, nlevels = 2)
                true_labs = l
                ...
                if counter % 1000 == 0:
                    loss_dev = []
                    acc_dev = []
                    for d, l in minibatches(dev_data, dev_labels, batchSize):
                        _, dialogue_lengthss = pad_sequences(d, 0)
                        word_idss, utterance_lengthss = pad_sequences(d, 0, nlevels = 2)
                        true_labs = l
                        labs_t, _ = pad_sequences(true_labs, 0)
                        dev_loss, dev_accuacy = sess.run([model.loss, model.accuracy], feed_dict = {model.word_ids: word_idss,
                                                                                                    model.utterance_lengths: utterance_lengthss,
                                                                                                    model.dialogue_lengths: dialogue_lengthss,
                                                                                                    model.labels:labs_t})
...
def preprocess(X):
    ...
    X = [[[word.lower() for word in utterance] for utterance in con] for con in X]

    #Noise Removal
    X = [[[scrub_words(word) for word in utterance] for utterance in con] for con in X]


    # Build lookuptable
    #adding all the words into this list
    set_of_all_words = {}
    for con in X:
        for utterance in con:
            for word in utterance:
                set_of_all_words[word]=0

    l = 0
    for i in sorted(set_of_all_words):
        set_of_all_words[i]=l
        l+=1
    #return lookuptable

    words_ids = [[[set_of_all_words[word] for word in utterance] for utterance in con] for con in X]
